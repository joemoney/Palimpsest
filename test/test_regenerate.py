...
ctx_holder = {"ctx": se.state_store.load_state("regeneratetest", se.state_store.DEFAULT_STORY_SLUG)}
...
ctx = ctx_holder["ctx"]
turn_count_after_first = ctx["state"]["pacing"]["turn_count"]
recent_turns_len_after_first = len(ctx["state"]["history"]["recent_turns"])
assert turn_count_after_first == 1
assert se._subplot_view(ctx, "subplot_001")["progress"] == 100
assert ctx["state"]["protagonist"]["flags"]["active"]["test_flag"] is True
assert "First narration." in ctx["state"]["history"]["recent_turns"][-1]
pending = ctx["state"]["pending_regenerate"]
...
ctx = ctx_holder["ctx"]
assert ctx["state"]["pacing"]["turn_count"] == turn_count_after_first, \
    "turn_count should net out the same (rolled back, then re-incremented), not double-counted"
assert len(ctx["state"]["history"]["recent_turns"]) == recent_turns_len_after_first, \
    "the regenerated turn should replace the last entry, not add a second one"
assert "Second narration (regenerated)." in ctx["state"]["history"]["recent_turns"][-1]
assert "First narration." not in ctx["state"]["history"]["recent_turns"][-1]
...
assert "test_flag" not in ctx["state"]["protagonist"]["flags"]["active"], \
